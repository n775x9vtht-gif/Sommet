...
import { IconSherpa, IconX, IconLock, IconCopy, IconCheck, IconPlus, IconTrash, IconConstruction } from './Icons';
...
const LeChantier: React.FC<LeChantierProps> = ({
  ...
  const selectedIdea = useMemo(() => savedIdeas.find((i) => i.id === selectedIdeaId), [savedIdeas, selectedIdeaId]);
  ...
  }, [selectedIdea, isGuestMode, onSaveKanban]);

  const handleMoveTask = (taskId: string, targetCol: KanbanTask['columnId']) => {
    if (!localBoard || !selectedIdea) return;

    const updatedTasks = localBoard.tasks.map((t) => (t.id === taskId ? { ...t, columnId: targetCol } : t));
  ...
  const handleColumnDrop = (e: React.DragEvent, colId: KanbanTask['columnId']) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  const getTasks = useCallback(
    (colId: KanbanTask['columnId']) => localBoard?.tasks.filter((t) => t.columnId === colId) || [],
    [localBoard]
  );
        ...
        <div key={index} className="space-y-3 my-2">
          {part.split('\n').map((line, lineIdx) => {
            ...
            if (
              ...
              return (
                <h3 key={key} className="font-bold text-lg text-gold-400 mt-5 mb-2">
            ...
            if (trimmedLine.startsWith('## ')) {
              return (
                <h3 key={key} className="font-bold text-xl text-white mt-5 mb-2">
                  {trimmedLine.replace('## ', '')}
                </h3>
              );
            }
            ...
            const markdownParts = line.split(/(\*\*.*?\*\*|`.*?`)/g);
            return (
              <p key={key} className="text-slate-300 leading-relaxed">
                {markdownParts.map((p, i) => {
                  if (p.startsWith('**') && p.endsWith('**')) {
                    return (
  ...
  if (savedIdeas.length === 0) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-center">
        ...
        <p className="text-slate-400">Générez une idée et un plan pour accéder au chantier.</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto animate-fade-in pb-20 h-[calc(100vh-100px)] flex flex-col">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between mb-8 gap-4 flex-shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm">Exécutez votre plan, tâche après tâche.</p>
          </div>
        </div>

        <div className="flex items-center bg-dark-800 p-2 rounded-2xl border border-dark-700 shadow-sm">
          <span className="text-xs font-bold text-slate-500 uppercase mr-3 ml-2">Projet :</span>
          <select
      ...
      {!localBoard ? (
        <div className="flex-1 flex items-center justify-center bg-dark-800/30 rounded-[2rem] border border-dark-700 border-dashed">
          <div className="text-center max-w-md p-8">
            ...
            <p className="text-slate-400 mb-6">Pour ouvrir le chantier, vous devez d'abord générer un Blueprint MVP pour ce projet.</p>
          </div>
        </div>
      ) : (
      ...
      {/* SHERPA CHAT MODAL - Updated Styles */}
      {sherpaModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center px-4">
          <div className="absolute inset-0 bg-dark-950/80 backdrop-blur-sm" onClick={() => setSherpaModalOpen(false)}></div>
          <div className="relative w-full max-w-2xl bg-dark-900 border border-gold-500/30 rounded-[2rem] shadow-2xl flex flex-col h-[85vh] max-h-[800px] animate-fade-in-up overflow-hidden">
            ...
            <div className="p-6 border-b border-dark-700 flex justify-between items-center bg-dark-800 shrink-0">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-gold-500 font-bold uppercase tracking-wide">Assistant Technique Senior</p>
                </div>
              </div>
              <button onClick={() => setSherpaModalOpen(false)} className="text-slate-500 hover:text-white transition-colors p-2 hover:bg-dark-700 rounded-lg" aria-label="Fermer le Sherpa">
            ...
            <div className="bg-dark-800/50 border-b border-dark-700 p-3 shrink-0">
              <div className="flex items-center gap-2 text-xs text-slate-400 px-2">
                <span className="font-bold uppercase text-slate-500">Contexte :</span>
                <span className="truncate max-w-[300px] md:max-w-[500px]">{currentTaskContext}</span>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 custom-scrollbar space-y-6 bg-dark-900">
              {chatMessages.length === 0 && isTyping && (
                <div className="flex flex-col items-center justify-center py-12">
                  ...
                  <p className="text-gold-500 animate-pulse font-medium">Le Sherpa analyse la situation...</p>
                </div>
              )}
              ...
              {chatMessages.map((msg, idx) => (
                <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  ...
                  >
                    {msg.role === 'sherpa' ? <div className="w-full">{renderMarkdown(msg.content)}</div> : msg.content}
                  </div>
                </div>
              ))}
              ...
              {isGuestMode && chatMessages.length > 0 ? (
                ...
                >
                  <div className="flex flex-col items-center justify-center gap-3">
                    ...
                    <p className="text-base font-bold text-white">Mode Conversationnel Verrouillé</p>
                    <p className="text-sm text-slate-400 max-w-sm mx-auto">Créez un compte gratuit pour discuter avec Le Sherpa et résoudre vos blocages.</p>
                    <span className="mt-3 inline-block text-xs font-bold text-dark-900 bg-gold-500 px-4 py-2 rounded-lg uppercase tracking-wide group-hover:bg-gold-400 transition-colors">Débloquer maintenant</span>
                  </div>
                </div>
              ) : (
                ...
                <form onSubmit={handleSendMessage} className="flex gap-4">
                  <input
                    ...
                    disabled={isTyping}
                    placeholder={chatMessages.length === 0 ? 'Posez votre question...' : 'Répondre au Sherpa...'}
...
const TaskCard: React.FC<{
  task: KanbanTask;
  onMove: (to: KanbanTask['columnId']) => void;
  onSherpa: () => void;
  onDelete: () => void;
  isGuestMode: boolean;
  ...
  return (
    ...
    >
      {/* Drop Indicator */}
      {isDragOver && !isDragging && <div className="absolute top-0 left-0 w-full h-1 bg-brand-500 rounded-t-xl z-10 shadow-[0_0_10px_rgba(59,130,246,0.5)]"></div>}
      ...
      <p className="text-sm text-slate-200 leading-relaxed mb-6 font-medium">{task.content}</p>
